...
OBR.onReady(async () => {
  ...
  if (!(svg instanceof SVGElement)) {
    throw new Error("cone-direction-picker.html is missing #compass");
  }

  const metadata = await OBR.tool.getMetadata(TOOL_ID);
  let selected = normalizeDirection(metadata.coneDirection);
  ...
  function updateCenter(direction, sub = "selected") {
    if (centerLabel) centerLabel.textContent = direction;
    if (centerSub) centerSub.textContent = sub;
  }
